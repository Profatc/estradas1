...
TOLERANCIA = 0.05  # Tolerância de 5 cm (0,05 m)
...
with col_left:
    ...
    if btn_min:
        checagens_min = [
            (user_l_abs, l_min_abs, "Mínimo Absoluto"),
            (user_l_otica, l_min_otica, "Fluência Ótica"),
            (user_l_conforto, l_min_conforto, "Conforto Centrífugo"),
            (user_l_rampa, l_min_rampa, "Máxima Rampa de Superelevação"),
            (user_lc_min_adotado, lc_min_arred, "Lc,min Final da Faixa (múltiplo de 10m)")
        ]
        
        erros_min = [
            f"• **{nome}**: inserido {val_user:.2f} m."
            for val_user, val_esp, nome in checagens_min
            if abs(val_user - val_esp) > TOLERANCIA
        ]
        ...
        if erros_min:
            ...
            st.write(f"* **Mínimo Absoluto:** {l_min_abs:.2f} m")
            st.write(f"* **Fluência Ótica:** {l_min_otica:.2f} m _({status_otica})_")
            st.write(f"* **Conforto Centrífugo (C={c_taxa:.3f}):** {l_min_conforto:.2f} m")
            st.write(f"* **Máxima Rampa de Superelevação ($1:{den_rmax:.1f}$):** {l_min_rampa:.2f} m")
...
with col_right:
    ...
    if btn_max:
        checagens_max = [
            (user_l_angulo, l_max_angulo, "Máximo Ângulo Central"),
            (user_l_tempo, l_max_tempo, "Tempo de Percurso"),
            (user_lc_max_adotado, lc_max_arred, "Lc,max Final da Faixa (múltiplo de 10m)")
        ]

        erros_max = [
            f"• **{nome}**: inserido {val_user:.2f} m."
            for val_user, val_esp, nome in checagens_max
            if abs(val_user - val_esp) > TOLERANCIA
        ]
        ...
        if erros_max:
            ...
            st.write(f"* **Máximo Ângulo Central ($L_c = R$):** {l_max_angulo:.2f} m")
            st.write(f"* **Tempo de Percurso ($2,2 \\cdot V$):** {l_max_tempo:.2f} m")
