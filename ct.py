...
l_min_conforto = (v_diretriz**3 / (46.656 * c_taxa * raio)) - ((e_r_dec * v_diretriz) / (0.367 * c_taxa)) if c_taxa * raio > 0 else 0.0
...
# --- 6. LANÇAMENTO DOS DADOS DO ALUNO (COLUNAS FIXAS) ---
...
# --- 7. BOTÃO DE CÁLCULO E MEMÓRIA DE ESTADO ---
st.divider()
...
if st.session_state.calcular_executado:
    ...
    tem_dados_min = any([user_l_abs > 0, user_l_otica > 0, user_l_conforto > 0, user_l_rampa > 0, user_lc_min_adotado > 0])
    tem_dados_max = any([user_l_angulo > 0, user_l_tempo > 0, user_lc_max_adotado > 0])
